        ...
        <div v-else>
            <nav class="navbar navbar-expand-lg navbar-dark bg-dark mb-3 sticky-top" style="padding-left: 20px">
                ...
                <router-link class="navbar-brand" to="/">
                    <img src="@/assets/images/branding/Camphouse-v2.png" alt="Camphouse Logo" height="30" width="30" />
                    <img src="@/assets/images/branding/Camphouse-Icon-Light.png" alt="Camphouse Logo" height="30"
                        width="150" />
                ...
                <div class="collapse navbar-collapse" id="navbarSupportedContent">
                    <ul class="navbar-nav ms-auto" v-if="!this.$store.state.loggedIn">
                        <li class="nav-item">
                            <router-link class="nav-link" to="/login"><i class="fa fa-user"></i> Login</router-link>
                        </li>
                        <li class="nav-item">
                            <router-link class="nav-link" to="/signup"><i class="fa fa-user-plus"></i> Sign
                                Up</router-link>
                        </li>
                        <li class="nav-item">
                    ...
                    <ul class="navbar-nav ms-auto" v-else>
                        ...
                        <li class="nav-item">
                            <router-link class="nav-link" to="/search"><i class="fa fa-search"></i> Search</router-link>
                        </li>
                        <li class="nav-item">
                            <router-link class="nav-link" :to="'/@' + this.$store.state.user.handle"><i
                                    class="fa fa-user"></i> Profile</router-link>
                        </li>
                        <li class="nav-item">
                            <router-link class="nav-link" to="/notifications"><i class="fa fa-bell"></i>
                                Notifications</router-link>
                        </li>
                        <li class="nav-item">
                            <!-- Dropdown for Additional Options -->
                            <div class="dropdown">
                                <button class="nav-link btn btn-secondary dropdown-toggle px-3" type="button"
                                    id="userOptions" data-bs-toggle="dropdown">
                                    Options
                                </button>
                                <div class="dropdown-menu dropdown-menu-end">
                                    <router-link class="dropdown-item" to="/admin"
                                        v-if="this.$store.state.user.admin"><i class="fa fa-user-shield"> </i>
                                        Admin</router-link>
                                    <router-link class="dropdown-item" to="/settings"><i class="fa fa-cog"></i>
                                        Settings</router-link>
                                    <p class="fake-link dropdown-item" @click="logout" v-if="!isLoading">
                        ...
                        <li class="nav-item">
                            <router-link class="nav-link btn btn-secondary" to="/new-post"><i class="fa fa-plus"></i>
...
export default {
    ...
    methods: {
        check_token() {
            ...
            // If the current path is the homepage, do not redirect
            if (currentPath === "/") {
                console.log("Homepage access allowed");
                return;
            }

            // Check if the current path is public
            const isPublicPath = publicPaths.includes(currentPath) || publicPathPatterns.some(pattern => pattern.test(currentPath));

            // If no token and not a public path, redirect to login
            if (!token || token == null) {
                console.log("No token found");
                if (!isPublicPath) {
                    const redirect = "/login?redirect=" + currentPath;
                    console.log(`Redirecting to login: ${redirect}`);
                    this.$router.push(redirect);
                    return; // Return to prevent further execution
                }
            } else {
                // If the user is logged in and trying to access a login or signup page, redirect to home
